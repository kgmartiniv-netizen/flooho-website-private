...
//
// /rest/api/3/search (used here originally) was removed by Atlassian —
// confirmed live via a 410 response pointing at this replacement,
// /rest/api/3/search/jql. Same jql= query param, still returns an
// "issues" array; only the path changed. maxResults=1 since this only
// needs to know whether any match exists, not the actual issues.
export async function findEpicByCompanyLabel(companySlug) {
  ...
  const url = `${process.env.JIRA_BASE_URL}/rest/api/3/search/jql?maxResults=1&jql=${encodeURIComponent(jql)}`;
